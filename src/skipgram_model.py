...
def load_embeddings(embedding_type):
    ...
    Args:
        embedding_type: Either 'cbow' or 'skipgram'
    """
    logging.info(f"Loading {embedding_type.upper()} embeddings...")
    
    try:
        # First try to load from PT file
        file_name = "cbow_2.pt" if embedding_type == 'cbow' else "skipgram_2.pt"
        ...
        file_name = "cbow_2.csv" if embedding_type == 'cbow' else "skipgram_2.csv"
...
def plot_metrics_comparison(results_dict):
    ...
    plt.title('Performance Metrics Comparison: CBOW vs Skip-gram')
    plt.ylim(0, 1)
    plt.xticks(rotation=45, ha='right')
    plt.grid(True, axis='y')
    plt.tight_layout()
    plt.savefig("../results/cbow_vs_skipgram_comparison.png")
    plt.close()
...
def plot_embedding_comparison(results_dict):
    ...
    # For each metric, compare CBOW vs Skip-gram across models
    for metric in metrics:
        plt.figure(figsize=(10, 6))
        
        # Prepare data
        cbow_values = [results_dict['cbow'][model][metric] for model in model_names]
        skipgram_values = [results_dict['skipgram'][model][metric] for model in model_names]
        
        x = np.arange(len(model_names))
        width = 0.35
        
        # Create the bars
        plt.bar(x - width/2, cbow_values, width, label='CBOW')
        plt.bar(x + width/2, skipgram_values, width, label='Skip-gram')
        
        plt.xlabel('Model')
        plt.ylabel(metric.capitalize())
        plt.title(f'{metric.capitalize()} Comparison: CBOW vs Skip-gram')
...
def process_embedding_type(embedding_type):
    """Process a specific embedding type (CBOW or Skip-gram)"""
...
def main():
    """Main function to run the classification pipeline for both embedding types"""
    logging.info("Starting embedding classification comparing CBOW and Skip-gram")
    ...
    results_df.to_csv("../results/cbow_vs_skipgram_comparison.csv")
