...
# === 2. Load CBOW Embeddings ===
def load_embeddings():
    """Load document embeddings from either PT or CSV files"""
    logging.info("Loading CBOW embeddings...")
    
    try:
        # First try to load from PT file
        data = torch.load("../datasets/interim/embeddings/pt/cbow_2.pt")
        ...
        csv_path = "../datasets/interim/embeddings/csv/cbow_2.csv"
        df = pd.read_csv(csv_path)
        logging.info(f"Successfully loaded embeddings from CSV: {csv_path}")
...
def train_model(model, X_train, y_train, X_test, y_test, 
    ...
    for epoch in range(num_epochs):
        ...
        with torch.no_grad():
            ...
            if val_loss < best_loss:
                ...
                patience_counter = 0
                # Save best model
                torch.save(model.state_dict(), "../models/best_cbow_classifier.pt")
            else:
                patience_counter += 1
    ...
    model.load_state_dict(torch.load("../models/best_cbow_classifier.pt"))
...
def main():
    ...
    logging.info("Starting CBOW embeddings classification")
    ...
    for model_info in models:
        ...
        pd.DataFrame(class_report).transpose().to_csv(
            f"../results/{name}_class_report.csv"
        )
        ...
        }, f"../models/{name}_cbow_classifier.pt")
    ...
    }).to_csv("../results/model_comparison.csv")
